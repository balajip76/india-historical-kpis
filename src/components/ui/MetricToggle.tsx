...
const ALL_OPTIONS: Option[] = [
  ...
  { value: 'cagr',      label: 'CAGR',      tooltip: 'Compound annual growth rate — 3 preset eras' },
...
export default function MetricToggle({ available, active, onChange }: Props) {
  ...
  return (
    <div className="flex items-center gap-0.5 bg-[#BCB8B1]/20 rounded-lg p-0.5 flex-wrap">
      {options.map((opt) => (
        <button
          key={opt.value}
          title={opt.tooltip}
          ...
          className={clsx(
            'px-2 py-1 text-[10px] sm:text-xs font-medium rounded-md transition-all duration-200 whitespace-nowrap',
            active === opt.value
