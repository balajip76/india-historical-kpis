...
const ALL_OPTIONS: Option[] = [
  ...
  { value: 'cagr', label: 'CAGR', tooltip: 'Compound annual growth rate over selected period' },
  { value: 'pct_gdp', label: '% GDP', tooltip: 'As a percentage of GDP' },
  { value: 'per_capita', label: 'Per Capita', tooltip: 'Value divided by population' },
];
...
export default function MetricToggle({ available, active, onChange }: Props) {
  ...
  return (
    <div className="flex items-center gap-1 bg-[#BCB8B1]/20 rounded-lg p-1">
      {options.map((opt) => (
        <button
          key={opt.value}
          title={opt.tooltip}
